...
namespace HMS.Modules.Matching.Application.Services
{
    ...
    public class StaffProposalService : IStaffProposalService
    {
            ...
            CancellationToken ct)
        {
            ...
            {
                foreach (var p in parameters) countCmd.Parameters.AddWithValue(p.ParameterName, p.Value ?? DBNull.Value);
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                // 1b. Hub ownership verification for Warehouse_Staff (skip for Driver proposals)
                ...
                if (shipment.Status != ShipmentStatus.Draft.ToString() && shipment.Status != ShipmentStatus.PendingReview.ToString())
                    throw new InvalidOperationException(
                        $"Shipment đang ở trạng thái {shipment.Status}. Chỉ có thể duyệt Proposal khi Shipment ở trạng thái Draft hoặc PendingReview.");

                // 3. Validate trip — skip for Driver proposals (no trip_post_id)
                ...
                {
                    // Driver proposal: validate the requested trip directly
                    var driverTrip = await ReadTripForUpdateAsync(conn, tx, proposal.RequestedTripId.Value, ct)
                        ?? throw new InvalidOperationException("Chuyến đi của tài xế không tồn tại.");

                    if (driverTrip.Status == "Completed" || driverTrip.Status == "Cancelled")
                        throw new InvalidOperationException("Chuyến đi đã hoàn thành hoặc bị hủy.");
                }
                ...
                if (proposal.CustomerId.HasValue)
                {
                    ...
                    {
                        _logger.LogWarning(ex, "Failed to save notification for proposal {ProposalId}", proposalId);
                    }
                }

                // 7. SignalR — notify customer if exists, or notify driver
                var targetUserId = proposal.CustomerId ?? proposal.DriverId;
                if (targetUserId.HasValue)
                {
                    try
                    {
                        await _dispatcher.SendProposalStatusToCustomerAsync(targetUserId.Value, new ProposalEventPayload
                        {
        ...
        private static async Task<bool> VerifyProposalHubOwnershipAsync(
            NpgsqlConnection conn, NpgsqlTransaction tx,
            Guid tripPostId, Guid expectedHubId, CancellationToken ct)
        {
            const string sql = """
                SELECT tp.created_by_staff_hub FROM transport.trip_posts tp
                WHERE tp.id = @tripPostId AND tp.is_deleted = FALSE;
            """;
            await using var cmd = new NpgsqlCommand(sql, conn, tx);
            cmd.Parameters.AddWithValue("tripPostId", tripPostId);
            var actualHub = await cmd.ExecuteScalarAsync(ct);
            return actualHub != null && actualHub != DBNull.Value && (Guid)actualHub == expectedHubId;
        }

        private static async Task<StaffProposalDetailDto> BuildProposalDetailFromReaderAsync(
            NpgsqlDataReader reader, Guid proposalId,
            ...
            // Paged query
            {
                ...
                var querySql = $"""
                    SELECT
                        ...
                        p.CreatedAt
                    FROM warehouse.Payments p
                    LEFT JOIN warehouse.quotations q ON q.id = p.QuotationId AND q.is_deleted = FALSE
                    LEFT JOIN warehouse.Shipments s ON s.id = p.ShipmentId AND s.is_deleted = FALSE
                    LEFT JOIN transport.trip_posts tp ON tp.id = (SELECT sp2.trip_post_id FROM warehouse.ShipmentProposals sp2 WHERE sp2.id = q.ProposalId) AND tp.is_deleted = FALSE
                    LEFT JOIN identity.users cu ON cu.id = p.CustomerId AND cu.is_deleted = FALSE
                    WHERE {whereSql}
