    ...
    public class StaffProposalService : IStaffProposalService
    {
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                var proposal = await ReadProposalForUpdateAsync(conn, tx, proposalId, ct)
                    ?? throw new InvalidOperationException("Proposal không t?n t?i.");
                ...
                var shipment = await ReadShipmentForUpdateAsync(conn, tx, proposal.ShipmentId, ct)
                    ?? throw new InvalidOperationException("Shipment không t?n t?i.");

                if (shipment.Status != ShipmentStatus.Draft.ToString() && shipment.Status != ShipmentStatus.PendingReview.ToString())
                    throw new InvalidOperationException(
                        $"Shipment dang ? tr?ng thái {shipment.Status}. Ch? có th? duy?t Proposal khi Shipment Draft ho\u00e3c PendingReview.");

                // 3. Validate capacity
                if (shipment.WeightKg <= 0 || shipment.VolumeCbm <= 0)
                    throw new InvalidOperationException("Weight và Volume pha3i ldbn hon 0.");
                ...
                if (proposal.TripPostId.HasValue)
                {
                    var tripPost = await ReadTripPostAsync(conn, tx, proposal.TripPostId.Value, ct)
                        ?? throw new InvalidOperationException("Trip Post không td1n ta1i.");
                    var trip = await ReadTripForUpdateAsync(conn, tx, tripPost.TripId, ct)
                        ?? throw new InvalidOperationException("Trip không td1n ta1i.");
                    if (trip.Status == "Completed" || trip.Status == "Cancelled")
                        throw new InvalidOperationException("Trip 11ã hoàn thành hoc7c bcb he7y.");
                }
                else if (proposal.RequestedTripId.HasValue)
                {
                    var trip = await ReadTripForUpdateAsync(conn, tx, proposal.RequestedTripId.Value, ct)
                        ?? throw new InvalidOperationException("Requested trip không td1n ta1i.");
                    if (trip.Status == "Completed" || trip.Status == "Cancelled")
                        throw new InvalidOperationException("Trip 11ã hoàn thành hoc7c bcb he7y.");
                }
                ...
                if (proposal.CustomerId.HasValue)
                {
                    ...
                    "Ð? xu?t dã du?c duy?t",
                    $"Ð? xu?t c?a b?n dã du?c ch?p nh?n và dang ch? báo giá.",
            ...
            string reason, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(reason))
                throw new InvalidOperationException("Lý do t? ch?i là b?t bu?c.");
            ...
            try
            {
                ...
                if (proposal.CustomerId.HasValue)
                {
                    try
                    {
                        await SaveNotificationAsync(conn, proposal.CustomerId.Value,
                    "Ð? xu?t b? t? ch?i",
                    $"Ð? xu?t c?a b?n dã b? t? ch?i. Lý do: {reason}",
                    "Proposal", proposalId, ct);
                    }
                    catch (Exception ex)
                    {
