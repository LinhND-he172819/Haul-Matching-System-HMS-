...
Write-Host "Driver=$($d.id) Vehicle=$($v.id)"

# Test: null payload - omit the field entirely
$body = @{
  driverId = $d.id
  vehicleId = $v.id
...
Write-Host "PAYLOAD (no scheduledDepartureAt field): $body"
try {
