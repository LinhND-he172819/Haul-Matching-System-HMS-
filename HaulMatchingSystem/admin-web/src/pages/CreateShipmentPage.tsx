...
export default function CreateShipmentPage({ onNavigate, proposalTripPostId, proposalTripId, pickupMode }: CreateShipmentPageProps) {
    ...
    const goToStep2 = () => {
        // DirectPickup: validate sender fields
        if (isDirectPickup) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            // Step 1: Create draft shipment (always needed)
            const data = await createDraftShipment({
                customerId: form.customerId,
                // Sender fields (DirectPickup only)
                senderName: isDirectPickup ? form.senderName || undefined : undefined,
                senderPhone: isDirectPickup ? form.senderPhone || undefined : undefined,
                pickupAddress: isDirectPickup ? form.pickupAddress || undefined : undefined,
                pickupLatitude: isDirectPickup && form.pickupLatitude ? Number(form.pickupLatitude) : undefined,
                pickupLongitude: isDirectPickup && form.pickupLongitude ? Number(form.pickupLongitude) : undefined,
                pickupNote: isDirectPickup ? form.pickupNote || undefined : undefined,
                // Cargo fields
            ...
            if (proposalWarning) {
                // Show warning after success screen renders
                setTimeout(() => alert(proposalWarning), 300);
            }
        } catch (err: any) {
                ...
                <div className="w-full max-w-lg bg-white rounded-xl shadow-[0_4px_15px_0_rgba(0,0,0,0.04)] p-5 flex flex-col items-center gap-4 text-center">
                    <div className="flex flex-col items-center gap-2 mb-4">
                        ...
                        <h1 className="text-4xl leading-[44px] tracking-[-0.02em] font-bold text-[#0b1c30]">
                            {isProposalMode ? 'Đề xuất đã gửi!' : 'Thành công!'}
                        </h1>
                        ...
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#eff4ff] rounded-full border border-[#c4c5d5]">
                            ...
                            <span className="text-xs font-medium text-[#444653]">
                                {isProposalMode ? 'Đề xuất ghép chuyến đã được gửi đến tài xế' : 'Chờ mang hàng đến Hub'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="w-full text-left">
                        <h2 className="text-xl leading-7 font-semibold text-[#0b1c30] mb-4">
                            {isProposalMode ? 'Các bước tiếp theo' : 'Các bước tiếp theo'}
                        </h2>
            ...
            <div className="w-full max-w-3xl bg-white rounded-xl shadow p-6 md:p-8">
                ...
                {step === 1 && (
                    <div className="space-y-8">
                        {/* Sender info section — only for DirectPickup */}
                        {isDirectPickup && (
                            <section className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                                <h2 className="text-xl font-semibold mb-1 flex items-center gap-2">
                                    <span className="material-symbols-outlined text-amber-600 text-[22px]">person_pin_circle</span>
                                    Thông Tin Người Gửi
                                </h2>
                                <p className="text-sm text-amber-700 mb-4">
                                    Tài xế sẽ đến nhận hàng tại địa chỉ của bạn.
                                </p>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between">
                            ...
                            <button
                                ...
                                disabled={loading}
                            >
                                {loading ? "Đang tạo..." : isProposalMode ? "Tạo đề xuất" : "Tạo đơn"}
                            </button>
                        </div>
                    </div>
                )}
